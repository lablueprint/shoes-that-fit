  ...
  function pushToAirtable() {
    ...
    base('Orders').create(
      ...
      (err, records) => {
        ...
        // go into the profile based on the profile username
        const past = profile.orders;
        console.log(past);
        let orders = [];
        orders = past.split(',');
        orders.push(id);
        ...
        base('Profile').select({ filterByFormula: `{Username} = '${username}'` }).all().then(
          (profileRecords) => {
            const profileId = profileRecords[0].getId();
            base('Profile').update([
              {
                id: profileId,
                fields: {
